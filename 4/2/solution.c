...
int main(void)
{
    ...
    // keep track of winning turn (idx of number sequence) for each board
    unsigned int win_turns[BOARD_CAP];
    size_t win_turns_size = boards_size;
    for (size_t i = 0; i < win_turns_size; i++) win_turns[i] = 0;

    // find the winning board and calculate
    for (size_t n = 0; n < numbers_size; n++)
    {
        for (size_t b = 0; b < boards_size; b++)
        {   
            if (win_turns[b] != 0) continue; // if board has already won, skip it
            for (size_t r = 0; r < GRID_SIZE; r++)
            {
                for (size_t c = 0; c < GRID_SIZE; c++)
                {
                    if (boards[b][r][c] == numbers[n])
                    {
                        ...
                        {
                            win_turns[b] = n;
                            printf("board %d won at turn %d with number %d\n", b, n, numbers[n]);
                        }
                    }
                }
            }
    ...
    // we have checked the whole thing, see who the last winner is
    size_t last_board_idx = 0;
    for (size_t i = 0; i < win_turns_size; i++)
        if (win_turns[i] > win_turns[last_board_idx]) last_board_idx = i;
     
    // get the sum
    size_t n_idx = win_turns[last_board_idx];
    unsigned int score = sum_of_unmarked_numbers(boards, last_board_idx, numbers, n_idx) * numbers[n_idx];
    printf("score of board %d: %d\n", last_board_idx, score);
